function filename = gentraincm(net,simfunc,bpfunc)

% CALCULATION
% ===========

% Copyright 1992-2002 The MathWorks, Inc.
% $Revision: 1.7 $  $Date: 2002/03/25 16:55:04 $
...
traincode = ...
  ...
  ['    fprintf(''\\n'')'];
  ['    if length(stop) fprintf(''TRAINC, %%s\\n\\n'',stop); end'];
  ...
  ['    '];
  ['    %% Performance'];
  ['    [Ac,N,Zl,Zi,Zb] = calca(net,Pdq,Aiq,1,TS);'];
  ['    Elq = calce(net,Ac,Tlq,TS);'];
  ['    X = getx(net);'];
  ['    perfq =' performFcn '(Elq,X,net.performParam);'];
  ['  '];
  };
